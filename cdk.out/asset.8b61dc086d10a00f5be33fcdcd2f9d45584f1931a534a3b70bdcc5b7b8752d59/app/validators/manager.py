...
def run_validators_for_evaluation(targets: List[Dict[str, Any]], region: str = None, account_id: str = None) -> List[Dict[str, Any]]:
    results = []
    if not targets:
        return results
    for t in targets:
        ...
        for v in validators:
            try:
                res = v.run(name=name, region=region, account_id=account_id, extra=t.get('extra'))
