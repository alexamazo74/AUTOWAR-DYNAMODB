...
def handler(event, context):
    ...
    print(f"evaluation_worker: Handler invoked with {len(event.get('Records', []))} records")

    for record in event.get('Records', []):
        body = record.get('body')
        try:
            logger.debug('Raw record body: %s', body)
            print(f"evaluation_worker: raw body: {body}")
            ...
            print(f"evaluation_worker: processing message: {json.dumps(msg if isinstance(msg, dict) else {'id': str(msg)})}")
            ...
            try:
                ...
                print(f"evaluation_worker: validator failed for {evaluation_id}")
                results = []
            ...
            try:
                ...
                print(f"evaluation_worker: failed to update evaluation item {evaluation_id}")
            ...
            for r in results:
                ...
                try:
                    ...
                    print(f"evaluation_worker: failed to write evidence item for {evaluation_id}")
            ...
            try:
                ...
                print(f"evaluation_worker: failed to write report metadata for {evaluation_id}")
            ...
            try:
                if REPORT_QUEUE_URL:
                    ...
                    print(f"evaluation_worker: enqueued report job {resp.get('MessageId')} for {evaluation_id}")
                else:
                    logger.debug('No REPORT_QUEUE_URL configured; skipping enqueue for %s', evaluation_id)
                    print(f"evaluation_worker: no REPORT_QUEUE_URL configured; skipping enqueue for {evaluation_id}")
            except Exception:
            ...
            print(f"evaluation_worker: unhandled exception processing record: {body}")
            continue

    logger.info('Handler completed, processed=%d', processed)
    print(f"evaluation_worker: Handler completed, processed={processed}")
