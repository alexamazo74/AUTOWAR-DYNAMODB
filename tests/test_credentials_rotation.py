...
def test_credentials_maintenance_rotation(monkeypatch):
    ...
    class FakeTable:
        ...
        def update_item(self, Key=None, UpdateExpression=None, ExpressionAttributeValues=None, ExpressionAttributeNames=None):
            self.updated.append({'Key': Key, 'UpdateExpression': UpdateExpression, 'Values': ExpressionAttributeValues})

    class FakeSecrets:
        def __init__(self):
            self.stored = {'SecretString': json.dumps({'iam_user': 'test-user', 'access_key_id': 'OLD', 'secret_access_key': 'OLD'})}
    ...
    monkeypatch.setattr('boto3.client', lambda service_name: fake_iam if service_name == 'iam' else fake_secrets)
