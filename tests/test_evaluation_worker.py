...
def test_evaluation_worker_processes_message(monkeypatch):
    ...
    monkeypatch.setattr(worker, 'dynamo', type('D', (), {'Table': lambda name: fake_eval_table if name == 'autowar-evaluations' else fake_evidence_table}))

    # monkeypatch validators runner to return a sample result
    def fake_run_validators(targets, region=None, account_id=None):
        return [{'name': 's3-public-access', 'resource': 'my-bucket', 'status': 'PASS', 'details': {'public_block': True}}]
