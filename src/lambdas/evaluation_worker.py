    ...
    for record in event.get("Records", []):
        ...
        try:
            ...
            logger.info(
                ...
                json.dumps(msg if isinstance(msg, dict) else {"id": str(msg)}),
            )
            ...
            try:
                if REPORT_QUEUE_URL:
                    ...
                    resp = sqs.send_message(
                        QueueUrl=REPORT_QUEUE_URL, MessageBody=json.dumps(payload)
                    )
                    logger.info(
                        "Enqueued report job %s for evaluation %s",
                        resp.get("MessageId"),
                        evaluation_id,
                    )
                else:
                    logger.debug(
                        "No REPORT_QUEUE_URL configured; skipping enqueue for %s",
                        evaluation_id,
                    )
            except Exception:
