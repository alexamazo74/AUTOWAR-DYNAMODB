...
async def validate_aws_credentials(request: AWSCredentialsValidationRequest):
    ...
    try:
        ...
        return {"success": False, "error": f"Error validando credenciales: {str(e)}"}
...
async def evaluate_security_real(request: AWSCredentialsValidationRequest):
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"[EVAL] Starting evaluation for account {request.account_id}, regions: {request.regions}"
        )
        ...
        logger.info(
            f"[EVAL] Evaluation complete: {eval_results['overall_score']}% score, {len(eval_results['questions'])} questions"
        )
        ...
        logger.info(
            f"[EVAL] Total findings: {len(all_findings)} (critical: {critical}, high: {high}, medium: {medium})"
        )
        ...
        return {"success": False, "error": f"Error evaluating security: {str(e)}"}
...
async def re_evaluate_best_practices(request: ReEvaluateBPRequest):
    ...
    try:
        ...
        logger.info(
            f"[RE-EVAL] Completed: {results['evaluated_count']} successful, {results['failed_count']} failed"
        )
        ...
        return {"success": False, "error": f"Error re-evaluating BP: {str(e)[:200]}"}
...
async def evaluate_security_mock():
    ...
    return {
        ...
        "summary": {
            ...
            "bps_evaluated": sum(
                q["bps_evaluated"] for q in mock_evaluation["questions_evaluated"]
            ),
        },
    }
...
@app.get(
    "/security/evaluations/{evaluation_id}/{question_id}"
...
@app.get(
    "/security/remediation/{evaluation_id}/{question_id}"
)  # Temporarily removed auth for testing
def get_security_remediation(evaluation_id: str, question_id: str):
    ...
    mock_plan = {
        ...
        "success_criteria": (
            "Todas las mejores prácticas de identidad y acceso implementadas con score >80"
            if question_id == "SEC01"
            else "Sistema de monitoreo completo activo con alertas configuradas"
        ),
    }
        ...
        "evaluations": [
            {
                ...
                "validation_results": {
                    "SEC01-BP01": {
                        ...
                        "description": "Proteger el usuario raíz",
                    },
                    "SEC01-BP02": {
                        "score": 100,
                        "status": "compliant",
                        "description": "Usar roles IAM",
                    },
                    "SEC01-BP03": {
                        "score": 50,
                        "status": "non-compliant",
                        "description": "Principio de menor privilegio",
                    },
                },
                "risks": [
                    {
