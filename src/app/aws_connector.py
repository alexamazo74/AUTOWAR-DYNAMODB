    ...
    def get_iam_users(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for user in response.get("Users", []):
                ...
                logger.info(
                    f"[AWS-CONN] User {user['UserName']}: MFA enabled = {user_data['mfa_enabled']}, Access keys = {len(user_data['access_keys'])}"
                )
            ...
            logger.info(
                f"[AWS-CONN] get_iam_users() completed successfully. Total users: {len(users)}"
            )
            return users
            ...
            logger.error(
                f"[AWS-CONN] Unexpected error getting IAM users: {str(e)}",
                exc_info=True,
            )
            raise
        ...
        try:
            ...
            for trail in response.get("trailList", []):
                ...
                try:
                    ...
                    trail_data["latest_delivery_time"] = str(
                        status_resp.get("LatestDeliveryTime", "")
                    )
                except ClientError:
            ...
            for recorder in recorders.get("ConfigurationRecorders", []):
                ...
                for s in status.get("ConfigurationRecordersStatus", []):
                    if s["name"] == recorder["name"]:
                        ...
                        recorder_data["last_status_change_time"] = str(
                            s.get("lastStatusChangeTime", "")
                        )
